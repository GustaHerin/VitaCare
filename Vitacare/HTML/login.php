...
if (isset($_GET['status'])) {
    if ($_GET['status'] == 'newuser') {
        $mensagem_feedback = "<p style='color: green; font-weight: bold; text-align: center;'>✅ Cadastro realizado! Faça login.</p>";
    } elseif ($_GET['status'] == 'error' && isset($_GET['code']) && $_GET['code'] == 'invalid_credentials') {
        $mensagem_feedback = "<p style='color: red; font-weight: bold; text-align: center;'>❌ Credenciais inválidas (E-mail ou Senha incorretos).</p>";
    }
}
?>
...
<head>
    ...
    <link rel="stylesheet" href="<?php echo $BASE_URL; ?>/CSS/sty_login.css">
...
<section id="borda">
    ...
    <div id="logo">
        <img src="<?php echo $BASE_URL; ?>/Img/logo_h.png" class="logo">
    </div>

    <div class="logotipagem">
        <h1>VitaCare Login</h1>
    ...
    <form action="../php/processo_login.php" method="POST">
        ...
        <div id= "lem-esq">
            <label>
                <input type="checkbox">
                Lembrar senha</label>
            <a href="recuperação.html">Esqueci senha</a>
        </div>

        <button id="login"  name="submit" placeholder="Login"> Login</button>
    </form>
